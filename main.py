...
for enemy in enemy_types:
    ...
    for animation in animation_types:
        ...
        temp_list = []
        #define the number of frames in the animation list
        num_of_frames = 20
...
class Castle():
    ...
    def draw(self):
        #check which image to used based on health
        if self.health <= 250:
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, image, x, y, angle):
        ...
        self.speed = 10
        #calculate the horizontal and vertical speeds of the angle
...
while run:
    ...
    if game_over == False:
        ...
        if tower_button.draw(screen):
            #check if there is enough money and build tower
            if castle.money >= TOWER_COST and len(tower_group) < max_towers:
                tower = Tower(tower_img_100, tower_img_50, tower_img_25, tower_position[len(tower_group)][0], tower_position[len(tower_group)][1], 0.2)
        ...
        if level_difficulty >= target_difficulty:
            ...
            for e in enemy_group:
                if e.alive == True:
                    enemies_alive += 1
            #if there are none alive then level is complete
